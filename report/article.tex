...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \begin{enumerate}
      ...
      \item Aguardar 1 segundo (cooldown)
    \end{enumerate}
  \end{enumerate}
\end{enumerate}
...
\begin{description}
  ...
  \item[Taxa de Compressão] Calculada pela fórmula: $100 \times \left(1 - \frac{\text{Tamanho comprimido}}{\text{Tamanho original}}\right)$. Valores maiores indicam maior eficiência.
...
\begin{table}[h]
  ...
  \begin{tabular}{lccc}
    ...
    \midrule
    gzip & 1448 $\pm$ 805 & 130 $\pm$ 73 & 62,65 $\pm$ 0,04 \\
    bzip2 & 1734 $\pm$ 964 & 869 $\pm$ 487 & 78,32 $\pm$ 0,28 \\
    xz & 10975 $\pm$ 6376 & 117 $\pm$ 57 & 88,12 $\pm$ 2,63 \\
    zstd & 141 $\pm$ 74 & 39 $\pm$ 19 & 75,95 $\pm$ 1,15 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\subsection{Análise das Regressões}

As regressões foram aplicadas usando tamanhos de 25 MB a 35 MB, faixa em que todos os algoritmos se comportaram de forma estável e comparável. Nos gráficos de tempo de compressão e tempo de descompressão, as curvas crescem de forma praticamente proporcional ao tamanho do arquivo, o que caracteriza uma relação quase linear --- por isso a regressão linear foi a escolha adequada para estimar tendências. Já na taxa de compressão, o comportamento não é linear: os algoritmos apresentam crescimento rápido no início e depois estabilizam, formando uma curva suavizada. Por isso, utilizou-se uma regressão SPLINE, que se ajusta melhor a curvas não lineares e capta essa variação de forma mais realista.

\subsection{Limitações do Estudo}

O estudo apresenta resultados robustos, mas possui algumas limitações importantes. A geração de arquivos por Cadeias de Markov --- embora realista --- ainda representa apenas dados textuais, o que restringe a generalização dos resultados para outros tipos de arquivo, como binários, imagens ou logs altamente redundantes. Além disso, todos os experimentos foram executados em uma única configuração de hardware relativamente modesta, o que significa que o comportamento dos algoritmos pode variar em máquinas com mais núcleos, maior banda de memória ou diferentes tipos de armazenamento. Outro ponto é que, apesar da correção metodológica na Fase 3, o uso apenas de níveis padrão de compressão (e não níveis ajustados) pode não refletir o desempenho real em cenários de produção onde parâmetros finos são ajustados manualmente.

\subsection{Recomendações e Implicações Práticas}

Com base nos resultados, recomenda-se que ambientes que priorizam velocidade adotem o zstd, que se mostrou extremamente eficiente tanto para compressão quanto para descompressão, mantendo excelente custo-benefício computacional. Para cenários onde a eficiência máxima de compressão é o objetivo --- como backup de longo prazo ou transferência em redes de baixa largura de banda --- o xz continua sendo a escolha ideal, desde que o tempo adicional de processamento não seja um problema. Em aplicações críticas em leitura (como pipelines de dados), deve-se evitar o bzip2 devido ao alto custo de descompressão. Além disso, o uso de warm-cache (pré-carregamento) e a padronização dos dados de teste, como demonstrado na Fase 3, podem ser incorporados como boas práticas de benchmarking em empresas e laboratórios para garantir reprodutibilidade e comparabilidade entre algoritmos.
